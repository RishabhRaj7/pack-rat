...
import { fmtDate, mapsDirectionsUrl, mapsUrl } from "@/lib/utils";
...
export function SharePage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      <div className="mb-6 rounded-3xl bg-primary-container p-6 text-on-primary-container">
        <h1 className="text-3xl font-semibold tracking-tight">{data.title}</h1>
        <p className="opacity-85">{data.city}, {data.country} · {fmtDate(data.startDate, "d MMM")} – {fmtDate(data.endDate)}</p>
        <Badge className="mt-3 bg-black/10 text-on-primary-container dark:bg-white/10">Read-only itinerary</Badge>
      </div>
      {data.hotel && (
      ...
      <div className="space-y-4">
        {data.days.map((d, i) => (
          <Card key={d.date} className="p-4">
            <div className="mb-3 flex items-baseline justify-between"><h2 className="text-lg font-semibold">Day {i + 1} <span className="text-sm font-semibold text-muted">· {fmtDate(d.date, "EEEE d MMMM")}</span></h2></div>
              ...
              <ol className="space-y-2">
                {d.places.map((p, j) => (
                  <li key={j} className="flex items-start gap-3 rounded-xl bg-surface-2 p-3">
                    ...
                    <div className="min-w-0 flex-1"><p className="font-semibold">{p.name} <span className="text-xs text-muted">{p.tags.map((t) => PLACE_TAGS.find((x) => x.value === t)?.label).join(" · ")}</span></p>{p.address && <p className="text-xs text-muted">{p.address}</p>}{p.notes && <p className="mt-1 text-sm text-muted">{p.notes}</p>}</div>
